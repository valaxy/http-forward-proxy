(function () {
	var orderIdMatch = location.href.match(/(?:\?|&)?__totoro_oid=([^&#]+)/)
	var laborIdMatch = location.href.match(/(?:\?|&)?__totoro_lid=([^&#]+)/)
	if (orderIdMatch && laborIdMatch) {
		var orderId = orderIdMatch && orderIdMatch[1]
		var laborId = laborIdMatch[1]
	} else {
		return
	}
	...
	function send(action, info) {
		...
		ajax({
			type       : 'POST',
			url        : 'http://www.wxtest.net:9999/__report', //todo, 强制指定url
